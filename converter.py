...
ORDERED_TOP_LEVEL_KEYS = [
    'env', 'service', 'pipeline', 'upstream_servers'
]

PIPELINE_ORDERED_KEYS = [
    'inputs', 'filters', 'parsers', 'multiline_parsers', 'outputs'
]
    ...
    while line_index < len(lines):
        ...
        if current_section_dict is not None:
            ...
            if len(key_value_parts) == 2:
                ...
                # Special handling for upstream_servers nodes
    ...
    for top_level_key in ORDERED_TOP_LEVEL_KEYS:
        if top_level_key == 'pipeline':
            ...
            for pipeline_key in PIPELINE_ORDERED_KEYS:
                ...
                for item_index, item in enumerate(config_data[pipeline_key]):
                    ...
                    for key, value in item.items():
                        ...
                        if item.get(f'{key}_block'):
                            ...
                            logger.debug(f"{pipeline_key}[{item_index}].{key}: block scalar")
                        else:
                            item_map[key] = value
                    if '_comment' in item:
            ...
            if pipeline_map:
                root_map['pipeline'] = pipeline_map
                logger.debug("Added 'pipeline' section")
        elif top_level_key == 'env':
            if top_level_key in config_data:
                env_sequence = CommentedSeq()
                for env_item in config_data[top_level_key]:
                    env_sequence.append(CommentedMap(env_item))
                root_map[top_level_key] = env_sequence
                logger.debug(f"Added {len(config_data[top_level_key])} env variables")
        elif top_level_key == 'service':
            if top_level_key in config_data:
            ...
            if top_level_key in config_data:
                ...
                for upstream_index, upstream_item in enumerate(config_data[top_level_key]):
                    upstream_map = CommentedMap({k: v for k, v in upstream_item.items() if k != '_comment' and k != 'nodes'})
                    ...
                    if '_comment' in upstream_item:
                        comment_token = CommentToken(f"# {upstream_item['_comment']}\n", CommentMark(0))
                        if upstream_index == 0:
                            upstream_sequence.yaml_set_start_comment(comment_token)
                        else:
                            upstream_sequence.yaml_set_comment_before_after_key(upstream_index, before=comment_token)
                    upstream_sequence.append(upstream_map)
                root_map[top_level_key] = upstream_sequence
                logger.debug(f"Added {len(config_data[top_level_key])} upstream servers")
...
def main() -> None:
    ...
    parser.add_argument('--output_file', help='Output YAML file path', default ='<input file>.yaml')
    parser.add_argument('--mappings', help='JSON mappings configuration file', default='mappings.json')
    ...
    if (args.output_file is None) or ((args.output_file) == '<input file>.yaml'):
        args.output_file = args.input_file + '.yaml'
        logger.info(f"Setting output file to {args.output_file}")
